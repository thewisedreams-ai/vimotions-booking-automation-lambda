...
from helpers_functions import read_excel_s3_with_lock
import boto3
import pandas as pd
...
try:
    dfs3 = read_excel_s3_with_lock(
        f"booking-automation-email-{environment}-bucket", "Actividades/Actividades.xlsx"
    )
    EMAIL_VAL = set(dfs3["Emails_Reservas"].dropna().str.strip())
    logger.info(f"Lista de emails cargados desde Excel S3 with locks: {EMAIL_VAL}")
except Exception as e:
    logger.error(f"Error al cargar el archivo Excel de S3: {e}")
